...
const events = [
  {
    size: "md:h-[180px] md:w-[120px] h-[50px] w-[50px]",
    textSize: "md:text-[9px] text-[4px]",
    date: null,
    link: "/singleevent",
  },
  {
    size: "md:h-[250px] md:w-[120px] h-[70px] w-[70px]",
    textSize: "md:text-[9px] text-[4px]",
    date: null,
    link: "/singleevent",
  },
  {
    size: "md:h-[300px] md:w-[120px] h-[80px] w-[80px]",
    textSize: "md:text-[14px] text-[5px]",
    date: "15th\nDec",
    link: "/singleevent",
  },
  {
      ...
      <div className="md:gap-8 gap-4 items-center justify-center px-4 md:flex hidden">
        {events.map((event, index) => (
          <Link
            href={event.link}
            key={index}
            className={`border rounded-full overflow-hidden relative ${event.size}`}
          >
              ...
              <div className="text-white text-center">
                <p
                  className={`font-bold font-Poppins uppercase ${
                    event.textSize
                  } ${event.date ? "" : "-rotate-90"}`}
                >
                  Corporate Event
                </p>
                {event.date && (
                  <div className="flex flex-col items-center">
                    <p className="font-Exo md:font-bold uppercase md:text-[20px] text-[5px] md:pt-4 whitespace-pre-line">
                      {event.date}
                    </p>
                    <MdAdsClick
                      className="flex justify-center ml-[13px] w-[10px] h-[10px] md:w-[25px] md:h-[25px]"
                      size={25}
                    />
                  </div>
                )}
